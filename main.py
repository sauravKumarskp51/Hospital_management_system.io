...
try:
    err=0
    mycon=mysql.connector.connect(host="localhost",port="3306" ,user="root",password="Saurav@4455",database="HOSPITAL",auth_plugin="mysql_native_password")
                ...
                if result==[]:
                    ...
                    if n1==fetch_1[0]:
                        if n3==fetch_1[2]:
                            ...
                            self.dialog = MDDialog(title = 'Success !',text = "Login Successful",size_hint = (0.7,0.2),buttons = [cancel_btn_username_dialogue])
                            self.dialog.open()
                        else:
                            cancel_btn_username_dialogue = MDFlatButton(text='Retry',on_release = self.close_username_dialogue)
